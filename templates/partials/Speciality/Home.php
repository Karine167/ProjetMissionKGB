                ...
                <table class="col-12 tabMissions">
                    ...
                    <?php foreach ($allElements as $element) { ?>
                        <tr>
                            ...
                            <td><?php echo(htmlspecialchars($element['name'])); 
                            if ($_GET['todo']=='delete'){ 
                                if ($element['id'] == $_GET['id'] ){ ?>
                                <p class="alert alert-danger"><?php echo('Supprimez cet élément ?'); ?>
                                <a href="/index.php?controller=back&action=Speciality&todo=delete&rep=oui&id=<?php echo($element['id']) ?>" class="btn btn-primary pt-2" aria-current="OUI">OUI</a>
                                <a href="/index.php?controller=back&action=Speciality&todo=home" class="btn btn-primary pt-2" aria-current="NON">NON</a></p>
                            <?php } } ?></td>
                            <td>
